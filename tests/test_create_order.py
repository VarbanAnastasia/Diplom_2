...
class TestCreateOrder:
    @allure.step("Тест на успешное создание заказа без авторизации с ингредиентами")
    def test_successful_order_creation_without_authentication_with_ingredients(self):
        response = requests.post(Constants.CREATE_ORDER, headers=Constants.headers,
                                 json=IngredientsPayload.data_ingredient)
        assert response.json()['success'] == True
        assert response.json()['name'] == "Бессмертный флюоресцентный бургер"

    @allure.step("Тест на создание заказа без авторизации без ингредиентов и проверку статуса")
    ...
    @allure.step("Тест на создание заказа без авторизации с неверным хешем ингредиентов и проверку статуса")
